...
import static core.Utils.isLessThanUnsigned;
/**
...
public class VarInt {
    ...
    public VarInt(ByteArrayInputStream bais) throws Exception {
        ...
        if (first < 253) {
            ...
            byte[] buf = new byte[2];
            bais.read(buf);
            val = (0xFF & buf[0]) | ((0xFF & buf[1]) << 8);
            originallyEncodedSize = 3;
        } else if (first == 254) {
            // 32 bits.
            byte[] buf = new byte[4];
            bais.read(buf);
            val = Utils.readUint32(buf, 0);
            originallyEncodedSize = 5;
        } else {
            // 64 bits.
            byte[] buf = new byte[8];
            bais.read(buf);
            val = Utils.readUint32(buf, 0) | (Utils.readUint32(buf, 4) << 32);
            originallyEncodedSize = 9;
        }
        this.value = val;
    }
